...
// Use ROM printf to log very early in startup before Arduino Serial is initialized.
extern "C" int esp_rom_printf(const char *fmt, ...);

void grbl_init() {
#ifdef ARDUINO
    esp_rom_printf("grbl_init: entry\n");
#else
    esp_rom_printf("grbl_init: entry\n");
#endif
#ifdef USE_I2S_OUT
    esp_rom_printf("grbl_init: before i2s_out_init\n");
    i2s_out_init();  // The I2S out must be initialized before it can access the expanded GPIO port
    esp_rom_printf("grbl_init: after i2s_out_init\n");
#endif
    esp_rom_printf("grbl_init: before wifi disable sequence\n");
    WiFi.persistent(false);
    WiFi.disconnect(true);
    WiFi.enableSTA(false);
    WiFi.enableAP(false);
    WiFi.mode(WIFI_OFF);
    esp_rom_printf("grbl_init: after wifi disable sequence\n");
    client_init();  // Setup serial baud rate and interrupts
    esp_rom_printf("grbl_init: after client_init\n");
    display_init();
    esp_rom_printf("grbl_init: after display_init\n");
    ...
    esp_rom_printf("grbl_init: after grbl_msg_sendf\n");
// show the map name at startup
#ifdef MACHINE_NAME
    report_machine_type(CLIENT_SERIAL);
#endif
    settings_init();  // Load Grbl settings from non-volatile storage
    esp_rom_printf("grbl_init: after settings_init\n");
    stepper_init();   // Configure stepper pins and interrupt timers
    esp_rom_printf("grbl_init: after stepper_init\n");
    system_ini();     // Configure pinout pins and pin-change interrupt (Renamed due to conflict with esp32 files)
    esp_rom_printf("grbl_init: after system_ini\n");
    init_motors();
    esp_rom_printf("grbl_init: after init_motors\n");
    memset(sys_position, 0, sizeof(sys_position));  // Clear machine position.
    machine_init();                                 // weak definition in Grbl.cpp does nothing
    esp_rom_printf("grbl_init: after machine_init\n");
    ...
    esp_rom_printf("grbl_init: after Spindle::select\n");
#ifdef ENABLE_WIFI
    WebUI::wifi_config.begin();
    esp_rom_printf("grbl_init: after wifi_config.begin\n");
#endif
#ifdef ENABLE_BLUETOOTH
    WebUI::bt_config.begin();
    esp_rom_printf("grbl_init: after bt_config.begin\n");
#endif
    WebUI::inputBuffer.begin();
    esp_rom_printf("grbl_init: complete\n");
}
